...
const orderMailPrePaid =async  (products,order) => {
    const updatedProduct = products && products.length > 0 && products.map((item) => {
        const productDetails = {productImage: item.images[0],productName: item.slug,price: item.price, productTitle: item.title}
...
const orderMailPostpaid =async  (data,order) => {
    const updatedProduct = order && order.length > 0 && order.map((item) => {
        const productDetails = {productImage: item.product.images[0],productName: item.product.title,price: item.product.price}
